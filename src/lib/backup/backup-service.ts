...
export class BackupService {
  ...
  private mapToBackupConfig(config: any): BackupConfig {
    return {
      ...
      sources: config.sources as BackupSource[],
      destination: config.destination as S3Destination,
      schedule: config.schedule as ScheduleConfig,
      options: config.options as BackupOptions,
