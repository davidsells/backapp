 ...
 * Routes to appropriate executor based on backup method
 */
export class BackupExecutor {
  ...
  async executeBackup(backupConfig) {
    ...
    if (backupMethod === 'rclone') {
      ...
      return rsyncExecutor.executeBackup(backupConfig);
    } else {
      this.logger.info(`Routing to archive executor for: ${backupConfig.name}`);
      return this.executeArchiveBackup(backupConfig);
    }
  }

  /**
   * Execute archive-based backup (original tar.gz method)
   * @param {object} backupConfig - Backup configuration from server
   */
  async executeArchiveBackup(backupConfig) {
